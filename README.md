...
### 📌 概述  
本项目展示了如何使用 **MNN 推理引擎**部署 **YOLOv11 实例分割模型**，支持 **x86** 和 **ARM64（如 RDK x5）** 两种硬件平台。通过预编译依赖库（Eigen、OpenCV、MNN），并利用 YOLO 官方导出功能生成 `.mnn` 模型，完成端到端推理与可视化。
...
- 输入 2：编译 ARM64（如 RDK x5）版本，生成build_arm64文件夹
